...
class AgentActivity(RecognitionHooks):
    ...
    def _save_conversation_to_json(self, role: str, content: str):
        """Save conversation to memory.json file"""
        try:
            ...
            logger.debug(f"Saved {role} message to memory.json: {content[:50]}...")
    ...
    def push_audio(self, frame: rtc.AudioFrame) -> None:
        ...
        # Only ignore audio if agent is speaking AND we're not in a state where we should be listening
        should_ignore_audio = (
            self._session.agent_state == "speaking" and 
            self._current_speech is not None and
            not self._current_speech.allow_interruptions
        )
        
        if should_ignore_audio:
            return

        # Always process audio when in listening state
        if self._session.agent_state == "listening":
            if self._rt_session is not None:
                self._rt_session.push_audio(frame)
            if self._audio_recognition is not None:
                self._audio_recognition.push_audio(frame)
        # Also process audio in thinking state if we allow interruptions
        elif (self._session.agent_state == "thinking" and 
            self._current_speech is not None and
    ...
    def push_video(self, frame: rtc.VideoFrame) -> None:
        """Enhanced video pushing with detailed diagnostics"""
        ...
        print(f"\n🎬 === PUSH_VIDEO CALLED ===")
        print(f"📹 Frame #{self._frames_received}")
        print(f"📹 Frame: {frame.width}x{frame.height}, type: {frame.type}")
        print(f"📹 Started: {self._started}")
        print(f"📹 RT Session: {self._rt_session is not None}")
        
        if not self._started:
            print("❌ push_video skipped - session not started")
            return

        if self._rt_session is not None:
            print(f"✅ Attempting to push frame to realtime session...")
            try:
                # Test if session is actually alive
                session_attrs = [attr for attr in dir(self._rt_session) if not attr.startswith('_')]
                print(f"🔍 Realtime session methods: {session_attrs[:10]}...")
                
                # Actual frame push
                self._rt_session.push_video(frame)
                print(f"✅ Frame successfully pushed to realtime session!")
                
                # Track successful pushes
                if not hasattr(self, '_frames_pushed'):
                    self._frames_pushed = 0
                self._frames_pushed += 1
                print(f"📊 Total frames pushed: {self._frames_pushed}")
                
            except Exception as e:
                print(f"❌ CRITICAL: push_video failed: {e}")
                import traceback
                traceback.print_exc()
        else:
            print("❌ push_video skipped - no realtime session available")
            # Debug why session is None
            if hasattr(self, 'llm'):
                print(f"🔍 LLM type: {type(self.llm)}")
                if isinstance(self.llm, llm.RealtimeModel):
                    print("🔍 LLM IS RealtimeModel but _rt_session is None!")
                    print(f"🔍 LLM capabilities: {self.llm.capabilities}")
                else:
                    print(f"🔍 LLM is NOT RealtimeModel: {type(self.llm)}")
    ...
    def _on_input_audio_transcription_completed(self, ev: llm.InputTranscriptionCompleted) -> None:
        ...
        if ev.is_final:
            ...
            # ADD THIS LINE: Save user message to JSON
            self._save_conversation_to_json("user", ev.transcript)
    ...
    async def _user_turn_completed_task(
        ...
        self._save_conversation_to_json("user", info.new_transcript)
    ...
    async def _tts_task(
        ...
        if add_to_chat_ctx:
            ...
            if forwarded_text:
                ...
                self._session._conversation_item_added(msg)
                self._save_conversation_to_json("assistant", forwarded_text)
        ...
        if self._session.agent_state == "speaking":
            self._on_agent_stop_speaking()


    def _on_agent_stop_speaking(self):
        # Only schedule a delayed transition if we are currently speaking
        if self._session.agent_state != "speaking":
            return

        # Don't clear audio buffers immediately - instead, let the natural
        # state transition handle it
        async def _delayed_listening():
            await asyncio.sleep(0.5)  # 500ms delay
            # Check again if we are still in the speaking state
            if self._session.agent_state == "speaking":
                # Only clear buffers after the delay
                if self._audio_recognition is not None:
                    self._audio_recognition.clear_user_turn()
                if self._rt_session is not None:
                    self._rt_session.clear_audio()
    ...
    async def _pipeline_reply_task(
        ...
        *,
        ...
        if text_out and text_out.text:
            ...
            self._save_conversation_to_json("assistant", text_out.text)
        ...
        if speech_handle.interrupted:
            ...
            if len(message_outputs) > 0:
                ...
                if audio_output is not None:
                    ...
                    msg_modalities =  msg_gen.modalities
