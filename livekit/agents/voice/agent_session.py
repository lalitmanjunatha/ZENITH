...
class AgentSession(rtc.EventEmitter[EventTypes], Generic[Userdata_T]):
    ...
    def _set_tts_active(self, active: bool):
    ...
    def say(self, text: str | AsyncIterable[str], *args, **kwargs) -> SpeechHandle:
        self._tts_active = True
        print(self._tts_active)  # Nova bolna start kar raha hai
        handle = self._activity.say(text, *args, **kwargs)

        def on_done(fut):
            self._tts_active = False  # Nova bolna finish kar gaya
    ...
    async def _forward_audio_task(self):
        ...
        async for frame in audio_input:
            if self._tts_active:
                logger.debug("Ignoring audio frame while TTS is active")
                # Audio frames ignored while Nova bol raha hai
                continue

            # Else, forward audio frame for processing (e.g., stt, vad, etc.)
            if self._activity:
                self._activity.push_audio(frame)



    async def safe_interrupt(self):
        """Interrupt the current speech generation.
    ...
    def _update_user_state(self, state: UserState, *, last_speaking_time: float | None = None) -> None:
        ...
        if state == "speaking":
            self._stt = True
            # interruption call hata diya, takey Nova ko interrupt na karein
        else:
            self._stt = False


    def user_speaking_detected(self):
        # Notify agent session that user started speaking
        self._update_user_state('speaking')

    def user_stopped_speaking(self):
        # Notify agent session that user stopped speaking
