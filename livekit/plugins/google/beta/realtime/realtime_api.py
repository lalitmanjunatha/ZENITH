...
class RealtimeSession(llm.RealtimeSession):
    ...
    # def _load_chat_context_from_db(self) -> llm.ChatContext:
    #     """
    #     Loads chat messages from the database and returns them as a list of Gemini API Content objects.
    #     """
    #     try:
    #         from nova_db import get_chat_history
    #         from google.genai import types  # Google's Gemini SDK types
    ...
    #                     logger.warning(f"Message {i} is not a dictionary: {type(message)}")
    #                     continue
    #                 if "role" not in message or "content" not in message:
    #                     logger.warning(f"Message {i} missing required fields: {message}")
    #                     continue

    #                 # Validate role
    #                 if message["role"] not in ["user", "assistant"]:
    #                     logger.warning(f"Message {i} has invalid role: {message['role']}")
    #                     continue

    #                 # Prepare Gemini Content object
    #                 gemini_turns.append(
    #                     types.Content(
    ...
    #                 logger.debug(f"Prepared Gemini message {i}: {message['role']} - {str(message['content'])[:50]}...")

    #             except Exception as e:
    #                 logger.error(f"Error processing message {i}: {e}")
    #                 continue
            
    #         logger.info(f"Successfully converted {len(gemini_turns)} messages to Gemini format")
    #         return gemini_turns
    #     except Exception as e:
    #         logger.error(f"Failed to load chat history from database: {e}")
    #         return []

    # # Also add this method to refresh chat context during session
    # async def refresh_chat_context(self) -> None:
    #     """Refresh chat context from database"""
    #     try:
    #         new_chat_ctx = self._load_chat_context_from_db()
            
    #         # Only update if we got something from database
    #         if new_chat_ctx.items:
    #             await self.update_chat_ctx(new_chat_ctx)
    #             logger.info("Chat context refreshed from database")
    #         else:
    #             logger.info("No new messages to refresh")
                
    #     except Exception as e:
    #         logger.error(f"Failed to refresh chat context: {e}")
    ...
    # In your RealtimeSession's push_video method
    def push_video(self, frame: rtc.VideoFrame) -> None:
        print(f"Pushing video frame: {frame.width}x{frame.height}, type: {frame.type}")
        ...
        try:
            ...
            self._send_client_event(realtime_input)
            print("Video frame sent to API")
            
        except Exception as e:
            print(f"Error encoding/sending video frame: {str(e)}")
    ...
    # In your RealtimeSession class

    @utils.log_exceptions(logger=logger)
    async def _main_task(self) -> None:
        """
        The main asynchronous task that manages the connection lifecycle with the Gemini Realtime API.
        """
            ...
            try:
                ...
                async with self._client.aio.live.connect(
                    ...
                    async with self._session_lock:
                        ...
                        # ==================================================
                        # START: RECONSTRUCT AND SYNC HISTORY
                        # ==================================================
                        
                        # try:
                        #     # Step 1: Directly load the user-only message history.
                        #     from nova_db import get_chat_history
                        #     import json
                        #     db_messages = get_chat_history()

                        #     if db_messages:
                        #         logger.info(f"Loaded {len(db_messages)} user messages. Reconstructing conversation history...")
                                
                        #         # Step 2: Reconstruct a two-sided conversation.
                        #         reconstructed_turns = []
                        #         for msg in db_messages:
                        #             # We only expect 'user' roles, but we'll be safe.
                        #             if msg.get('role') == 'user':
                        #                 try:
                        #                     content_str = msg.get('content', '[]')
                        #                     text_parts = json.loads(content_str)
                        ...
                        #                         # Add a generic, placeholder 'model' turn right after.
                        #                         # This creates a valid conversational flow.
                        #                         reconstructed_turns.append(
                        #                             types.Content(role="model", parts=[types.Part(text="Acknowledged what we have talked early.")])
                        #                         )
                        #                 except (json.JSONDecodeError, TypeError):
                        #                     continue # Skip malformed records

                                # Step 3: Send the reconstructed, two-sided history to the server.
                                # if reconstructed_turns:
                                #     logger.info(f"Sending {len(reconstructed_turns)} reconstructed turns to the Gemini server.")
                                #     await session.send_client_content(
                                #         turns=reconstructed_turns,
                                #         turn_complete=False, # This is context.
                                #     )
                                #     logger.info("Reconstructed historical context sent successfully.")
                                # else:
                                #     logger.warning("DB messages were loaded, but no valid turns could be reconstructed.")

                        #     else:
                        #         logger.info("No message history found in the database to sync.")

                        # except Exception as e:
                        #     logger.error(f"An unexpected error occurred during history sync: {e}", exc_info=True)

                        # ==================================================
                        # END: RECONSTRUCT AND SYNC HISTORY
                        # ==================================================

                # ... (rest of the function continues as before)

                        # ==================================================
                        # END: SELF-CONTAINED HISTORY SYNC LOGIC
                        # ==================================================

                            # ==================================================
                            # END: HISTORICAL CONTEXT BLOCK
                            # ==================================================

                    # After setting up the context, create tasks to handle real-time communication.
                    ...
                    restart_wait_task = asyncio.create_task(self._session_should_close.wait(), name="gemini-restart-wait")
                    ...
                    # If a task failed, log the exception and re-raise it to trigger the retry logic.
                    for task in done:
                    ...
                    # If the session was closed intentionally, break the loop.
                    if restart_wait_task not in done and self._msg_ch.closed:
                        break

                    # Clean up any pending tasks before restarting the loop.
                    for task in pending:
                        await utils.aio.cancel_and_wait(task)

            except asyncio.CancelledError:
                # If the task is cancelled, exit the loop gracefully.
                break
            except Exception as e:
                # Handle connection errors and retry logic.
                logger.error(f"Gemini Realtime API error: {e}", exc_info=e)
                if not self._msg_ch.closed:
                    # Do not retry if the initial connection fails (e.g., bad API key).
                ...
                # Ensure the active session is closed before the loop ends or restarts.
                await self._close_active_session()
        ...
        if model_turn := server_content.model_turn:
            for part in model_turn.parts or []:
                ...
                if part.inline_data:
                    try:
                    # Trigger GUI effect
                        EffectPlugin.trigger_effect()
                    except Exception as e:
                        logger.warning(f"Error triggering Effects {str(e)}")

                    if model_turn := server_content.model_turn:
                        for part in model_turn.parts or []:
                            if part.inline_data:
                                # Set flag when model starts speaking
    ...
    def _mark_current_generation_done(self) -> None:
        ...
        if gen.input_transcription:
            ...
            # from nova_db import store_message
            # store_message(
            #     role="user",
            #     content=gen.input_transcription,
            #     message_id=gen.input_id
            # )
            # print("Assistant message added to database")
