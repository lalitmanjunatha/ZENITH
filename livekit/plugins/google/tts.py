...
class ChunkedStream(tts.ChunkedStream):
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        try:
            if self._tts._on_speech_start:
                self._tts._on_speech_start()
            ...
            if self._tts._on_speech_end:
                self._tts._on_speech_end()
    ...
    async def _run_stream(
        ...
        try:
            ...
            async for resp in stream:
                if first_chunk and self._tts._on_speech_start:
                    # Trigger effect on first audio chunk
                    self._tts._on_speech_start()
                    first_chunk = False
