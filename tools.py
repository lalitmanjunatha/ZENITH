...
DB_PATH = "zenith_memory/chat_history.db"
...
async def send_whatsapp_message(contact: str, message: str) -> str:
    ...
    try:
        ...
        return (
            ...
            f"🧠 क्या कुछ और भेजना है sir? जवाब दें — Zenith उस संदेश को भेज देगा।"
        )
...
#     VOICE COMMAND: "Zenith, I want to analyze a dataset"
#     """
...
#                             <p>Example voice command: "Zenith, analyze dataset with target sales"</p>
#                         </div>
#                     `;
#                 }}
#             }}
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                ...
                <p>Advanced Groundwater Analysis Engine v2.0 • Powered by Zenith</p>
            </div>
        </div>
...
# @function_tool()
# async def generate_image_from_prompt(prompt: str, output_path: str = "zenith_generated.png") -> str:
#     """
...
#         output_path: Path to save the generated image (default = "zenith_generated.png").
...
@function_tool()
async def ml_train_classification(data_path: str, target_column: str) -> str:
    ...
    try:
        ...
        dp.clean_data()
        X_train, X_test, y_train, y_test = dp.train_test_split(target_column)
...
@function_tool()
async def ml_train_regression(data_path: str, target_column: str) -> str:
...
@function_tool()
async def ml_cluster(data_path: str, n_clusters: int = 3) -> str:
...
@function_tool()
async def ml_anomaly_detection(data_path: str, contamination: float = 0.1) -> str:
...
@function_tool()
async def nlp_sentiment(text: str) -> str:
...
@function_tool()
async def nlp_extract_entities(text: str) -> str:
...
@function_tool()
async def nlp_summarize(text: str, max_length: int = 130) -> str:
...
@function_tool()
async def nlp_detect_language(text: str) -> str:
...
@function_tool()
async def nlp_preprocess(text: str) -> str:
...
async def knowledge_search(query: str, top_k: int = 5) -> str:
    """Search your indexed personal files for relevant information."""
    try:
...
@function_tool()
async def knowledge_index(directory: str = None) -> str:
    """Index files from a directory into your personal knowledge base."""
    try:
        from zenith_brain import ZenithBrain
        ...
        scan_dir = directory or str(Path.home() / "Documents")
        ...
        return (
            ...
            f"?? Skipped: {result.get('skipped', 0)} files"
        )
    except Exception as e:
...
async def knowledge_ask(question: str) -> str:
    """Ask a question about your personal data and files."""
    try:
        from zenith_brain import ZenithBrain
...
@function_tool()
async def content_search(keyword: str, top_k: int = 5) -> str:
    """Search your personal files by content to find relevant information."""
    try:
        from zenith_brain import ZenithBrain
        ...
        results = brain.rag_pipeline.search_by_content(keyword, top_k=top_k)

        if not results:
            return f"? No content found matching '{keyword}' in your indexed files."

        output = f"?? Found {len(results)} result(s) for '{keyword}':\n\n"
        for i, r in enumerate(results, 1):
            file_name = r.get("file_name", r.get("source_file", "unknown"))
            score = r.get("similarity_score", 0)
            chunk_text = r.get("text", r.get("chunk_text", ""))[:200]
            output += f"{i}. ?? {file_name} (score: {score:.3f})\n   {chunk_text}\n\n"
...
@function_tool()
async def open_file_by_content(query: str) -> str:
    """Find and open a file based on its content, not its name."""
    try:
        from zenith_brain import ZenithBrain
        ...
        result = brain.rag_pipeline.find_and_open_file(query)
        ...
        if result.get("found"):
            ...
            try:
                ...
                return f"? Found and opened: {file_path}"
            except Exception as e:
                return f"?? Found file but could not open it: {file_path}\nError: {str(e)}"
        else:
...
@function_tool()
async def show_folder_contents(folder_path: str = None) -> str:
    """Show the contents of a folder, including file names and sizes."""
    try:
        from pathlib import Path

        folder = Path(folder_path or str(Path.home()))
        ...
        return f"? Failed to list folder: {str(e)}"


@function_tool()
async def search_by_filename(name_query: str) -> str:
    """Search for files by name across your indexed directories."""
    try:
        from zenith_brain import ZenithBrain
        ...
        results = brain.rag_pipeline.search_files_by_name(name_query)

        if not results:
            return f"? No files found matching '{name_query}'"

        output = f"?? Found {len(results)} file(s) matching '{name_query}':\n\n"
        for r in results:
            output += f"  ?? {r.get('name', 'unknown')}\n"
            output += f"     Path: {r.get('path', 'unknown')}\n"
            output += f"     Size: {r.get('size_bytes', 0) / 1024:.1f} KB\n\n"

        return output
    except Exception as e:
        return f"? File name search failed: {str(e)}"
